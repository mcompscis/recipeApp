...
SELECT COUNT(DISTINCT T.recipe_id, T.recipe_name, T.num_ratings, T.avg_rating, T.description, T.img_url) AS CNT
FROM
(SELECT tag_text,
	...
	WHERE RI.recipe_id NOT IN (
			...
			INNER JOIN RecipeIngredient RI 
			ON I.ingredient_id = RI.ingredient_id
			WHERE ((%(is_excluded_ingr_lst_null)s IS NOT NULL) AND (I.ingredient_name IN %(exclude_ingredients)s))
		)
            AND ((%(is_included_ingr_lst_null)s IS NULL) OR (ingredient_name IN %(include_ingredients)s))) r
